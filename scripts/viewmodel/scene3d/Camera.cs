...
namespace ProceduralFoliageGenerator.ViewModel
{
    public partial class Camera : Camera3D
    {
        ...
        /// </summary>
        public Vector2 Uv { get; set; } = new Vector2(Single.Pi / 2, -Single.Pi / 2);
        
        /// <summary>
        /// Relative changes made to the look direction of the camera since last input. 
        /// </summary>
        public Vector2 DeltaUv { get; set; } = new Vector2(0,0);
        ...
        public void TranslateCamera(Vector3 velocity)
        {
            var globalTranslation = velocity.Y * Vector3.Up -
                                    ...
                                    new Vector3(LookDirection.X,0,LookDirection.Z).Normalized().Cross(Vector3.Up) * velocity.X; 
            
            this.GlobalTranslate(globalTranslation);            
        } 
        ...
        public override void _Process(double delta)
        {
            ...
            {
                this.TranslateCamera(Velocity * (float)delta * SpeedModifier);
            }

            if (DeltaUv != Vector2.Zero)
            {
                var x = Uv.X; var y = Uv.Y;
                x = x + DeltaUv.X * (float)delta;
                y = (float)Math.Clamp(y + DeltaUv.Y * (float)delta, 0.3f, 3.11f);
                ...
                this.LookAt(this.Position + LookDirection);
