...
public partial class GenerationExecutor : Node
{
    ...
    ///     Not used for persistent storage of the outputs, rather just as an intermediate solution until the values are logged
    ///     and cleared.
    /// </summary>
    ...
    /// <summary>
    ///     Determines whether the standard output and error are logged after the execution of the procedural generator
    ///     program.
    /// </summary>
    ...
    /// </summary>
    public static GenerationExecutor Instance
    {
        get
        {
            if (_instance is null) _instance = new GenerationExecutor();
            return _instance;
        }
    ...
    {
        var callableExec = new Callable(this, MethodName._ExecuteThread);
    ...
    private void LogOutput()
    {
        if (SaveLogs)
        {
            ...
            {
                foreach (string item in OutputLog) logFile.StoreLine(item);
                logFile.Close();
            }
            else
            {
