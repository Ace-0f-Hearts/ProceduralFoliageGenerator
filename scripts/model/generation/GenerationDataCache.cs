...
{
    public static String CacheLocation { get; private set; } = "user://GenerationCache"; 
    public static void SaveFiles(GenerationCommandData data)
    {
        var time = Time.GetDatetimeDictFromSystem();
        
        var cacheInstanceLocation = CacheLocation + "/" + time["year"] + "-" + time["month"] + "-" + time["day"] +  "-"  + time["hour"] + "-" + time["minute"] + "-";
